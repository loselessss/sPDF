...
- PDF 호환 옵션으로 저장된 Adobe Illustrator(`.ai`) 파일을 파일 선택창, 최근 파일, 드래그앤드롭, 명령행 실행, 페이지 구성과 PDF 병합에서 열 수 있습니다.
- Windows 설치 프로그램이 기본 앱을 강제로 바꾸지 않고 `.ai` 파일의 사용자별 **연결 프로그램** 목록에 sPDF를 추가합니다.

### 개선

- Illustrator 파일을 수정한 뒤 저장하면 `.ai` 원본과 Illustrator 전용 데이터에 덮어쓰지 않고 항상 PDF 저장 위치를 선택하도록 했습니다.
- PDF 호환 데이터가 들어 있지 않은 Illustrator 파일은 지원 조건을 명확하게 안내합니다.

## 1.12.1 - 2026-08-24

### 개선

- 기본 README와 변경 이력을 영어로 제공하고, 각 문서 상단에서 별도의 한국어판으로 이동할 수 있게 했습니다.
- **도움말 → 언어**에서 영어와 한국어를 선택할 수 있게 했습니다. 변경한 언어는 sPDF를 다시 실행하면 적용됩니다.
- 업데이트 창에서 설정된 UI 언어와 일치하는 릴리스 변경 내용을 표시합니다. 언어 구분이 없는 이전 릴리스의 변경 내용도 기존처럼 표시합니다.
