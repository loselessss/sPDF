...
- [ ] **안티앨리어싱 품질:** GPU 경로에서 안티앨리어싱이 적용되지 않는 것처럼 보이는 증상을 재현한다. 100%·폭 맞춤·800% 및 Windows 100/125/150/200% 배율에서 글자 윤곽, 사선·곡선, 클리핑·마스크 경계와 이미지 확대 품질을 CPU 기준 화면과 비교한다. Direct2D AA 설정, geometry realization 허용 오차, DPI/변환 좌표, 중간 bitmap·합성·보간 경로를 점검한다. 벡터 AA와 이미지 보간을 구분하고 PDF의 명시적 Interpolate 설정을 무조건 덮어쓰지 않는다.
