...
- 1.29.4는 투명 배경 이미지 마스크의 범위를 이미지 변환에서 계산하고, soft-mask와 geometry clip의 중간 bitmap을 현재 target과 교차하는 실제 device-space bounds로 제한한다. 잘라낸 원점은 중첩 캡처별로 행렬에 반영하고 합성 시 원래 위치로 복원한다. 같은 ACB 측정에서 retained scene 제출 중앙값은 210.29 ms에서 47.67 ms로 약 4.41배 개선됐다.
